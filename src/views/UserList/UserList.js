...
import { UsersToolbar, UsersTable , UserActivateDialog } from './components';
...
const UserList = (props) => {
  ...
  const filterUsers = (search, column, dir, length, page , date , star) => {
  ...
  const onFilter = (search, column, dir, length , date , stars) => {
    filterUsers(search, column, dir, length , null , date , stars);
  }

  const setUserPage = (page) => {
    filterUsers(filterData.search , filterData.column , filterData.dir, filterData.length , page);
  }
  return (
    <div className={classes.root}>
      {showLoading ? (
        ...
        <Fragment>
          <UsersToolbar onFilter={onFilter} is_picker={is_picker}/>
          {is_picker ? '' : (
          ...
          <Dialog open_dialog={openStarDialog}
            ...
            on_positive_btn={() => starUser(selectedUser.id , selectedUser.star == 1 ? 0 : 1)}
          />
          <div className={classes.content}>
            <UsersTable is_picker={is_picker} refresh={fetchUsers} is_manage={true} paginate={paginate_info} setPage={setUserPage} onUserPick={onUserPick} users={users} toggleActivate={toggleActivate}  toggleStarred={toggleStarred} userInfoClick={onUserClick}/>
          </div>
        </Fragment>
      )}
    </div>
