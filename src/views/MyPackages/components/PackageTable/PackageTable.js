...
const PackageTable = props => {
    ...
    console.log('props.match.params.user_id',props.match.params.user_id)
    React.useEffect(() => {
                ...
                <PerfectScrollbar>
                  <div className={classes.inner}>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {packages.slice(0, parseInt(paginate.per_page)).map(mPakcage => (
                          <TableRow
                            className={classes.tableRow}
                            hover
                            key={mPakcage.id}
                            // onClick={() => showDetails(window.dashboard_url +  (user_id ? "/reports/users/"+user_id+"/booking/"+mPakcage.id : "/mypackages/"+mPakcage.id))}
                          >
                            
                            <TableCell>{mPakcage.staff_package.package.name}</TableCell>
                            <TableCell>{mPakcage.staff_package.staff.user.name + " " + mPakcage.staff_package.staff.user.family}</TableCell>
                            <TableCell>{mPakcage.staff_package.duration } {Lang.my_packages.list.minutes}</TableCell>
                            <TableCell>{mPakcage.schedules_count}</TableCell>
                            ...
                            <TableCell>
                                { parseInt(mPakcage.paid) == 1 ? (
                                  ...
                                  to={window.dashboard_url +  (user_id ? "/reports/users/"+user_id+"/booking/"+mPakcage.id : "/mypackages/"+mPakcage.id)}
                              >
...
export default withRouter(PackageTable);
